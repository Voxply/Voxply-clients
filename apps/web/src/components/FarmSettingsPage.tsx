...
function GeneralTab({ farmUrl }: { farmUrl: string }) {
  ...
  async function handleSave() {
    ...
    try {
      const updated = await patchFarmSettings(farmUrl, payload);
      setSettings(updated);
...
function HubsTab({ farmUrl }: { farmUrl: string }) {
  ...
  async function handleSuspend(hub: FarmHubEntry, reason: string) {
    ...
    setActionError(null);
    try {
      await suspendFarmHub(
        farmUrl,
        hub.id,
        hub.suspended_at === null,
        reason || null,
      );
      setHubs((prev) =>
        prev.map((h) =>
  ...
  async function handleDelete(hub: FarmHubEntry) {
    ...
    setActionError(null);
    try {
      await deleteFarmHub(farmUrl, hub.id);
...
function UsersTab({ farmUrl }: { farmUrl: string }) {
  ...
  async function fetchPage(p: number) {
    ...
    setActionError(null);
    try {
      const result = await getFarmUsers(farmUrl, p, 50);
      if (p === 1) {
  ...
  useEffect(() => { void fetchPage(1); }, [farmUrl]);

  async function handleRevokeSessions(pubkey: string) {
    setActionError(null);
    try {
      await revokeFarmUserSessions(farmUrl, pubkey);
      setUsers((prev) =>
        prev.map((u) =>
